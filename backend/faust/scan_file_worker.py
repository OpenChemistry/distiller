...
from ncempy.io import dm, ser, emd

import faust
from config import settings
from constants import (DATE_DIR_FORMAT, TOPIC_HAADF_FILE_EVENTS,
                       TOPIC_SCAN_METADATA_EVENTS, TOPIC_SCAN_FILE_EVENTS)
from faust_records import ScanMetadata
from utils import update_scan, get_scan, ScanUpdate
...
async def generate_image_from_data(tmp_dir: str, data_path: str, image_filename: str) -> AsyncPath:
...
async def copy_to_ncemhub(src_path: AsyncPath, dest_path: AsyncPath):
    ...
    dest_path = AsyncPath(settings.HAADF_NCEMHUB_DM4_DATA_PATH) / date_dir / src_path.name
    ...
    async with aiohttp.ClientSession() as session:
        async for event in haadf_events:
            ...
            with tempfile.TemporaryDirectory() as tmp:
                await copy_to_ncemhub(AsyncPath(path),  AsyncPath(settings.HAADF_NCEMHUB_DM4_DATA_PATH))
...
scan_file_events_topic = app.topic(TOPIC_SCAN_FILE_EVENTS, value_type=ScanFileUploadedEvent)
...
async def watch_for_scan_file_events(scan_file_events):
    ...
    async with aiohttp.ClientSession() as session:
        async for event in scan_file_events:
            ...
            id = event.id
            with tempfile.TemporaryDirectory() as tmp:
                await copy_to_ncemhub(AsyncPath(path), AsyncPath(settings.NCEMHUB_DATA_PATH))
