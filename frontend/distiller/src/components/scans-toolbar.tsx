...
type FilterPopoverProps = {
  ...
  const FilterPopover:  React.FC<FilterPopoverProps> = (props) => {
    ...
    useEffect(() => {
      if ((!isNull(startDate) && !startDate.isValid) || (!isNull(endDate) && !endDate.isValid)) {
        return;
      }
  ...
  export const ScansToolbar: React.FC<ScansToolbarProps>  = (props) => {
    ...
    return (
      ...
      <Box>
        <Tooltip title="Filter Scans">
          <Button
            ...
            size="small"
            color="primary"
            startIcon={<Badge color="primary"  variant="dot" invisible={!showFilterBadge}><FilterListIcon /></Badge>}
          >
            Filter
          </Button>
        </Tooltip>
      ...
      <FilterPopover anchorEl={filterAnchorEl} onClose={onFilterClose} onFilter={onFilter}/>
      <ExportMenu anchorEl={exportAnchorEl} onClose={onExportClose} onExport={onExport} />
    </Toolbar>
    );
  };
