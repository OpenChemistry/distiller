...
from fastapi import APIRouter, Depends, HTTPException, Response, File, UploadFile, Form, status
...
from app.kafka.producer import (send_remove_scan_files_event_to_kafka,
                                send_scan_event_to_kafka, send_scan_file_event_to_kafka)
...
def  generate_sha256(form_data: schemas.ScanFromFileMetadata):
    ...
    if len(form_data.locations) >  0:
        ...
        logger.warn(f"No location available for scan. Using only created time and microscopy.")
...
async def create_scan_from_file(db, meta: schemas.ScanFromFileMetadata, file_upload: UploadFile, ser_file_upload: Optional[UploadFile]):
    sha = generate_sha256(meta)

    # Check for existing scan with this sha
    if crud.get_scans_count(db, sha=sha) != 0:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Scan in SHA already exists")
...
@router.post("", summary="Note: This API either take a JSON body for creating a 4D scan or a scan files. We are parsing the request manually, the OpenAPI documentation is not correct.", response_model=schemas.Scan, response_model_by_alias=False)
async def create_scan(request: Request,
    ...
    try:
        ...
        if content_type == "application/json":
            ...
            if file is None:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid request, scan file is required" )
            scan_metadata_str = form_data.get("scan_metadata")
            if scan_metadata_str is None:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid request, scan metadata is required" )

            scan_metadata = schemas.ScanFromFileMetadata.parse_raw(scan_metadata_str)

            print(file.filename)
            file_stem = Path(file.filename).stem.replace('%20', '\\ ')
            print(type(file_stem))
            print(file_stem == '15.39.34\\ Scanning\\ Acquire.ser')
            ser_file = form_data.get(f"{file_stem}.ser")
            print(f"ser: {ser_file}")
            scan = await create_scan_from_file(db, scan_metadata, file_upload=file, ser_file_upload=ser_file)
        else:
