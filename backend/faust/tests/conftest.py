...
from faust_records import Scan, Location
...
@pytest.fixture
def cori_machine():
    return { "name": "cori", "account": "m3795", "qos": "realtime", "nodes": 20, "constraint": "haswell", "cpus_per_task": 64, "bbcp_dest_dir": "${DW_JOB_STRIPED}", "balance_bridge": "ncem"}

@pytest.fixture
def perlmutter_machine():
    return { "name": "perlmutter", "account": "staff", "qos": "science", "nodes": 8, "constraint": "gpu", "cpus_per_task": 128, "ntasks_per_node": 1, "bbcp_dest_dir": "$PSCRATCH/test"}

@pytest.fixture
def expected_cori_submission_script():
    excepted_cori_submission_script_path = Path(__file__).parent / "fixtures" / "cori_submission_script"
...
def expected_perlmutter_submission_script():
    excepted_perlmutter_submission_script_path = Path(__file__).parent / "fixtures" / "perlmutter_submission_script"
